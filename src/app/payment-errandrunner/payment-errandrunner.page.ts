...
export class PaymentErrandrunnerPage implements OnInit {
  ...
  constructor(private jobService: JobService, private jobERService: JobERService, private route: ActivatedRoute,
    ...
    this.jobERService.getCompletedJobsById(this.errandId)
      .then(data => {
        this.job = data;
        ...
        this.Bill = new Payment(this.errandId, this.job.price, parseFloat((this.job.price * 0.1).toFixed(2)), this.job.price - parseFloat((this.job.price * 0.1).toFixed(2)), '', 'Received')
      })
